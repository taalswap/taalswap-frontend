...
function Swap({
  ...
  const getAddressBySymbol = useCallback(
    (symbol: string | undefined) => {
      ...
      const curToken =
        curSymbol !== undefined
          ? TOKEN_LIST.tokens.find((token) => token.symbol === curSymbol && token.chainId.toString() === chainId)
          : null
  ...
  const handleSwap = useCallback(() => {
    if (priceImpactWithoutFee && !confirmPriceImpactWithoutFee(priceImpactWithoutFee, storedLangCode)) {
      return
    }
    if (!swapCallback) {
      return
      ...
      <SwapBody>
        <Wrapper id="swap-page">
          ...
          <CardChildBody>
            ...
            {trade !== undefined ? <AdvancedSwapDetailsDropdown trade={trade} /> : null}
