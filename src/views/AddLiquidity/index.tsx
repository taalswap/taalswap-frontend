...
import { ChainId, Currency, currencyEquals, ETHER, KLAYTN, BINANCE, POLYGON, AURORA, TokenAmount, WETH } from 'taalswap-sdk'
...
export default function AddLiquidity({
  ...
  async function onAdd() {
    ...
    const router = getRouterContract(chainId, library, account, true)
    ...
    if (currencyA === ETHER || currencyA === KLAYTN || currencyA === BINANCE || currencyA === POLYGON || currencyA === AURORA || currencyB === ETHER || currencyB === KLAYTN || currencyB === BINANCE || currencyB === POLYGON || currencyB === AURORA) {
      const tokenBIsBNB = currencyB === ETHER || currencyB === KLAYTN || currencyB === BINANCE || currencyB === POLYGON || currencyB === AURORA
          ...
          addTransaction(response, {
            summary:
              storedLangCode === 'ko-KR'
                ? `${parsedAmounts[Field.CURRENCY_A]?.toSignificant(3)} ${currencies[Field.CURRENCY_A]?.symbol}${t(
                    'Aand',
                  )} ${parsedAmounts[Field.CURRENCY_B]?.toSignificant(3)} ${currencies[Field.CURRENCY_B]?.symbol}${t(
                    'AddB',
                  )}`
  ...
  const pendingText =
    storedLangCode === 'ko-KR'
      ? `${parsedAmounts[Field.CURRENCY_A]?.toSignificant(6)} ${currencies[Field.CURRENCY_A]?.symbol}${t(
          'Aand',
        )} ${parsedAmounts[Field.CURRENCY_B]?.toSignificant(6)} ${currencies[Field.CURRENCY_B]?.symbol}${t('SupplyB')}`
  ...
  const handleCurrencyBSelect = useCallback(
    (currB: Currency) => {
      ...
      if (currencyIdA === newCurrencyIdB) {
        ...
        history.push(`/add/${curChainId}/${currencyIdA || ethStr}/${newCurrencyIdB}`)
      }
    },
