...
const RecentXSwapTransactionsModal = ({ onDismiss = defaultOnDismiss }: RecentTransactionsModalProps) => {
  ...
  const getSymbolByAddress = useCallback((address: string | undefined, chain: string) => {
    let curChainId
    ...
    const curToken =
      address !== undefined
        ? TOKEN_LIST.tokens.find((token) => token.address === address?.toLowerCase() && token.chainId === curChainId)
        : null
  ...
  const getUrl = (txHash, chainAddress, fromChain) => {
    const urlChainId = Object.keys(BRIDGE_ADDRESS).find((key) => BRIDGE_ADDRESS[key] === chainAddress)
  ...
  const getSummary = (
    ...
    return (
      ...
      >
        ...
        <Link style={{ marginRight: '0.3rem' }} href={getUrl(txHash, to, fromChain)} target="_blank">{`${parseFloat(
  ...
  return (
    <Modal title={t('Recent X-Swap transactions')} onDismiss={onDismiss} style={{ position: 'relative' }}>
      {!account && (
