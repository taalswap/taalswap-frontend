    ...
    <AutoColumn gap="md" style={{ marginTop: '20px' }}>
      {tradeX !== undefined && chainId !== crossChain ? (
        <>
          ...
          <RowBetween align="flex-end">
            <RowFixed gap="0px">
              <CurrencyLogo currency={tradeX?.outputAmount.currency} size="24px" style={{ marginRight: '12px' }} />
              <Text
                fontSize="24px"
                style={{ marginLeft: '10px', fontWeight: 500 }}
                color={
                  priceImpactSeverity > 2
                    ? theme.colors.failure
                    : showAcceptChanges && tradeX?.tradeType === TradeType.EXACT_INPUT
                    ? theme.colors.primary
                    : 'text'
                }
              >
            ...
            {tradeX.tradeType === TradeType.EXACT_INPUT ? (
              <PriceInfoText>
                {storedLangCode === 'ko-KR' ? (
                  <>
                    <span>
                      {trade.inputAmount.currency.symbol}-{trade.outputAmount.currency.symbol}-
                      {tradeX.outputAmount.currency.symbol}
                    </span>
                    {' 의 추정 교환값입니다. 수령하게 될 최소량은 '}
                    <span>
                      {tradeX?.outputAmount.toSignificant(6)} {tradeX.outputAmount.currency.symbol}
                    </span>
                    {
